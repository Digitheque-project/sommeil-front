...
#
# Sur Render (runtime Docker), les variables d'environnement du service sont
# passées automatiquement au build en tant que build args — mais UNIQUEMENT
# celles déclarées ici par un ARG. Toute variable NEXT_PUBLIC_* absente de
# cette liste retombe sur la valeur par défaut codée dans lib/config.ts.
...
ARG NEXT_PUBLIC_CHU_SERVICE_URL
ARG NEXT_PUBLIC_SERVICE_REGISTRY_URL
ARG NEXT_PUBLIC_DOSSIER_PATIENT_URL
ARG NEXT_PUBLIC_ACCUEIL_URL
ARG NEXT_PUBLIC_NOTIFICATION_HUB_URL
ARG NEXT_PUBLIC_NOTIFICATION_SERVICE_ID
ARG NEXT_PUBLIC_CHU_ID
ARG NEXT_PUBLIC_SLEEP_SERVICE_ID
ENV NEXT_PUBLIC_SOMMEIL_API_URL=$NEXT_PUBLIC_SOMMEIL_API_URL \
    ...
    NEXT_PUBLIC_CHU_SERVICE_URL=$NEXT_PUBLIC_CHU_SERVICE_URL \
    NEXT_PUBLIC_SERVICE_REGISTRY_URL=$NEXT_PUBLIC_SERVICE_REGISTRY_URL \
    NEXT_PUBLIC_DOSSIER_PATIENT_URL=$NEXT_PUBLIC_DOSSIER_PATIENT_URL \
    NEXT_PUBLIC_ACCUEIL_URL=$NEXT_PUBLIC_ACCUEIL_URL \
    NEXT_PUBLIC_NOTIFICATION_HUB_URL=$NEXT_PUBLIC_NOTIFICATION_HUB_URL \
    NEXT_PUBLIC_NOTIFICATION_SERVICE_ID=$NEXT_PUBLIC_NOTIFICATION_SERVICE_ID \
    NEXT_PUBLIC_CHU_ID=$NEXT_PUBLIC_CHU_ID \
    NEXT_PUBLIC_SLEEP_SERVICE_ID=$NEXT_PUBLIC_SLEEP_SERVICE_ID \
