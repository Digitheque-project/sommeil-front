...
import type { PsgInterpretation } from "@/lib/api/psg-interpretations";
...
export default function InterpretationPsgPage() {
  ...
  const isValidating = createMutation.isPending || updateMutation.isPending || validateMutation.isPending;
  ...
  /**
   * Bouton unique : enregistre le contenu (création ou mise à jour selon
   * qu'un brouillon existe déjà) puis valide et signe immédiatement.
   */
  const validateInterpretation = async () => {
    if (!selectedExam || !draft.trim()) return;
    try {
      let interpretationId = selectedInterpretation?.id;
      ...
      {selectedExam && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          >
            ...
            {!isLocked && draft.trim() && (
              <div className="flex items-center justify-end border-t border-outline-variant bg-surface-bright px-6 py-5">
                <ActionButton
                  permission={selectedInterpretation ? "psg:interpret_validate" : "psg:interpret_create"}
