...
export default function ConsultationTraitementClient({
  ...
  const goToParacliniques = () => {
    ...
    params.set("prescripteurId", consultation.medecinId);
  ...
  const finishConsultation = async () => {
    ...
    const nonMedicaments = {
      ...
      rdvMotif: followUpSaved ? "" : followUp.motif,
      rdvNiveau: followUp.niveau,
      rdvDate: followUp.date,
      hospitalisationMotif: hospitalisationStatus ? "" : hospitalisation.motif,
      hospitalisationService: hospitalisationStatus ? "" : hospitalisation.service,
    };
  ...
  const saveFollowUp = async () => {
    if (!consultationId || (!followUp.motif.trim() && !followUp.date)) return;
    setSavingFollowUp(true);
    try {
      await consultationApi.traiterConsultation(consultationId, "controle", { rdvMotif: followUp.motif || null, rdvDate: followUp.date || null, rdvNiveau: followUp.niveau });
  ...
  const cancelAndGoBack = async () => {
    if (consultationId) { try { await consultationApi.traiterConsultation(consultationId, "annuler"); } catch { /* retour conservé même si le service est indisponible */ } }
      ...
      <div className="mx-auto max-w-7xl animate-fade-in">
        ...
        <section className="mt-7 overflow-hidden rounded-[30px] border border-slate-100 bg-white shadow-[0_4px_20px_rgba(17,17,26,.05)]">
          <div className="flex overflow-x-auto border-b border-slate-100 px-6">
            ...
            <button type="button" onClick={goToParacliniques} className="ml-auto my-2 flex shrink-0 items-center gap-2 rounded-xl bg-[#EAF3FA] px-4 py-2 text-[12px] font-bold text-[#006A8C] hover:bg-[#D1E5F5]"><span className="material-symbols-outlined text-[18px]">biotech</span> Examens para-cliniques <span className="material-symbols-outlined text-[16px]">arrow_forward</span></button>
          </div>
