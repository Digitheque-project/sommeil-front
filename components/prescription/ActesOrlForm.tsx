...
import { creerPrescriptionActesOrl } from '@/lib/prescription-api';
import { useToast } from "@/components/ToastProvider";

type Urgence = "n" | "u" | "tu";
const urgenceClasses: Record<Urgence, string> = { n: "un", u: "uu", tu: "utu" };
const TYPES_ACTES_ORL = ["Lavage auriculaire", "Extraction de bouton de cérumen", "Extraction de corps étranger", "Biopsie"];
...
interface DemandeItem { id: number; typeActe: string; renseignements: string; remarques: string }

export default function ActesOrlForm({ patient, prescripteur, onAddToCart }: Props) {
  ...
  const typeEffectif = typeActe === "Autre" ? typeAutre.trim() : typeActe;
  const currentDemandeValid = renseignements.trim() !== "" && typeEffectif !== "";
  const canSubmit = cart.length > 0;

  function addToCart() {
    setCart(prev => [...prev, { id: Date.now(), typeActe: typeEffectif, renseignements, remarques }]);
    setRenseignements(""); setTypeActe(""); setTypeAutre(""); setRemarques("");
  }

  function buildDemandes(items: DemandeItem[]) {
    return items.map(i => ({ typeActe: i.typeActe, renseignementsCliniques: i.renseignements, remarques: i.remarques }));
  }

  async function handleSubmit() {
    setShowModal(false); setLoading(true);
    try {
      await creerPrescriptionActesOrl({ patientId: patient.id, prescripteurId: prescripteur.id, chuId: prescripteur.chuId, serviceId: prescripteur.serviceId, urgence, alertes, demandes: buildDemandes(cart) });
      showSuccess(`${cart.length} acte(s) ORL envoyé(s)`);
    ...
    <div>
      <div className="g2-form mb12">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="card" style={{ padding: 12 }}><label className="lbl">Renseignements cliniques <span className="req">*</span></label><textarea rows={3} value={renseignements} onChange={e => setRenseignements(e.target.value)} placeholder="Motif, détails cliniques de l'acte ORL..." /></div>
          <div className="card" style={{ padding: 12 }}>
            <div className="mb12"><label className="lbl">Type d&apos;acte <span className="req">*</span></label><div className="rg">{TYPES_ACTES_ORL.map(t => <label className="rc" key={t}><input type="radio" name="actes-orl-type" value={t} checked={typeActe===t} onChange={()=>setTypeActe(t)} /><span>{t}</span></label>)}<label className="rc"><input type="radio" name="actes-orl-type" value="Autre" checked={typeActe==="Autre"} onChange={()=>setTypeActe("Autre")} /><span>Autre</span></label></div>{typeActe==="Autre" && <input type="text" style={{marginTop:8}} placeholder="Précisez..." value={typeAutre} onChange={e => setTypeAutre(e.target.value)} />}</div>
            <label className="lbl">Remarques (pour cet acte)</label>
            ...
            >
              ...
              Ajouter cet acte à la prescription
            </button>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div className="card" style={{ padding: 12 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
              <span className="sh" style={{ margin: 0 }}>Actes prescrits</span>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {cart.map(item => (
                  <div key={item.id} style={{ display: "flex", alignItems: "flex-start", gap: 8, background: "var(--navy-lt)", border: "1.5px solid var(--navy-mid)", borderRadius: 9, padding: "8px 10px" }}>
                    <span style={{ flex: 1, fontSize: 12, color: "var(--navy)", fontWeight: 600, lineHeight: 1.4 }}>{item.typeActe}</span>
