...
import { creerPrescriptionPolysomnographie } from '@/lib/prescription-api';
...
interface ValidatedPrescription { urgence: Urgence; alertes: string; renseignements: string; remarques: string; patient: Props["patient"] & { age: number | null; sexeLabel?: string }; prescripteur: Props["prescripteur"]; date: string; }

export default function PolysomnographieForm({ patient, prescripteur, onAddToCart }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      await creerPrescriptionPolysomnographie({ patientId: patient.id, prescripteurId: prescripteur.id, chuId: prescripteur.chuId, serviceId: prescripteur.serviceId, urgence, alertes, renseignements, remarques });
      ...
      showSuccess("Demande Polysomnographie transmise"); setUrgence("n"); setAlertes(""); setRenseignements(""); setRemarques("");
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="card" style={{ padding: 12 }}><label className="lbl">Renseignements cliniques <span className="req">*</span></label><textarea rows={3} value={renseignements} onChange={e => setRenseignements(e.target.value)} placeholder="Contexte clinique, indications pour la polysomnographie..." /></div>
          <div className="card" style={{ padding: 12 }}><label className="lbl">Remarques</label><textarea rows={3} value={remarques} onChange={e => setRemarques(e.target.value)} placeholder="Informations spécifiques..." /></div>
        </div>
      ...
      {showModal && <div className="mb op" onClick={e => { if (e.target === e.currentTarget) setShowModal(false); }}><div className="mbox"><h3>Confirmer ?</h3><p>La demande de polysomnographie sera transmise au service.</p><div className="mbtns"><button className="bca" onClick={()=>setShowModal(false)}>Annuler</button><button className="bok" onClick={() => { if (onAddToCart) { const snap = { patientId: patient.id, prescripteurId: prescripteur.id, chuId: prescripteur.chuId, serviceId: prescripteur.serviceId, urgence, alertes, renseignements, remarques }; onAddToCart({ label: `Polysomnographie`, count: 1, submit: () => creerPrescriptionPolysomnographie(snap) }); setShowModal(false); } else { handleSubmit(); } }}>Confirmer</button></div></div></div>}
